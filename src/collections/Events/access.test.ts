...
const fieldsToFill: Pick<Event, 'startDate' | 'format' | 'content' | 'eventType'> = {
...
describe('Events access', () => {
  ...
  describe('site users can...', async () => {
    ...
    test('not read not-their Events', async ({ tid, testUser, events }) => {
      ...
      await Promise.all(
        notTheirEvents.map(async (event) => {
  ...
  describe('site users with multiple sites can...', async () => {
    ...
    test('create a Event for all their sites, upon site selection', async ({
      tid,
      testUser,
      sites,
    }) => {
    ...
    test('delete a Event for all their sites, upon site selection', async ({
      tid,
      testUser,
      sites,
      events,

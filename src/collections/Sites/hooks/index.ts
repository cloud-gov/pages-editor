...
export const createSiteSinglePages: CollectionAfterChangeHook<Site> = async ({
  ...
  if (operation === 'create' && transactionID) {
    ...
    try {
      ...
      console.error('Error generating single pages:', error)
      throw error
    }
  }
}
    ...
    if (BUCKET && process.env.NODE_ENV !== 'test') {
      ...
      const bot = (
        await payload.find({
          ...
          limit: 1,
          where: {
            and: [{ 'sites.site.id': { equals: doc.id } }, { 'sites.role': { equals: 'bot' } }],
          },
          req,
        })
      ).docs[0]
      ...
      try {
        ...
        if (error instanceof S3ServiceException) {
          console.error(`Error from S3 while uploading object. ${error.name}: ${error.message}`)
        } else {
          throw error
        }
      }

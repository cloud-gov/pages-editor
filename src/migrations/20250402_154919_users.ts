...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_users_sites_role" AS ENUM('manager', 'user', 'bot');
  ...
  CREATE TABLE IF NOT EXISTS "users_sites" (
  	...
  	"site_id" integer,
  	"role" "enum_users_sites_role" DEFAULT 'user'
  );
  ...
  ALTER TABLE "users" ADD COLUMN "selected_site_id" numeric NOT NULL;
  ...
  DO $$ BEGIN
   ALTER TABLE "users_sites" ADD CONSTRAINT "users_sites_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."users"("id") ON DELETE cascade ON UPDATE no action;
  EXCEPTION
   WHEN duplicate_object THEN null;
  END $$;
  ...
  CREATE UNIQUE INDEX IF NOT EXISTS "sites_name_idx" ON "sites" USING btree ("name");
  ...
  CREATE INDEX IF NOT EXISTS "users_sites_order_idx" ON "users_sites" USING btree ("_order");
  CREATE INDEX IF NOT EXISTS "users_sites_parent_id_idx" ON "users_sites" USING btree ("_parent_id");
  CREATE INDEX IF NOT EXISTS "users_sites_site_idx" ON "users_sites" USING btree ("site_id");
  ...
  CREATE INDEX IF NOT EXISTS "posts_site_idx" ON "posts" USING btree ("site_id");
  CREATE INDEX IF NOT EXISTS "_posts_v_version_version_site_idx" ON "_posts_v" USING btree ("version_site_id");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  ALTER TABLE "users_sites" DISABLE ROW LEVEL SECURITY;
  ...
  ALTER TABLE "_pages_v" DROP CONSTRAINT IF EXISTS "_pages_v_version_site_id_sites_id_fk";

  ALTER TABLE "posts" DROP CONSTRAINT IF EXISTS "posts_site_id_sites_id_fk";

  ALTER TABLE "_posts_v" DROP CONSTRAINT IF EXISTS "_posts_v_version_site_id_sites_id_fk";
  ...
  DROP INDEX IF EXISTS "posts_site_idx";
  DROP INDEX IF EXISTS "_posts_v_version_version_site_idx";
  DROP INDEX IF EXISTS "payload_locked_documents_rels_sites_id_idx";
  ...
  ALTER TABLE "_pages_v" DROP COLUMN IF EXISTS "version_site_id";
  ...
  ALTER TABLE "_posts_v" DROP COLUMN IF EXISTS "version_site_id";
  ...
  ALTER TABLE "users" DROP COLUMN IF EXISTS "selected_site_id";
  ...
  DROP TYPE "public"."enum_users_sites_role";

import type { Access, FieldAccess, CollectionSlug, GlobalSlug, Where } from 'payload'
import type { Post, User, Site, Event, Media, News, History } from '@/payload-types'
...
export function getAdminOrSiteUser(
  ...
  const adminOrSiteUser: Access<Post | User | Site | Event | Media | News> = async ({
    req,
    data,
  }) => {
    let { user } = req
...
export function getAdminOrSiteUserGlobals(
  ...
  const adminOrSiteUser: Access<History> = async ({ req }) => {
    let { user } = req
    if (!user) return false
    if (user.isAdmin) return true

...
describe('Published Build Status', () => {
    ...
    test('Should Create a new published build status if it does not exists and non required parameters are null', async () => {
        const mockCreate = vi.fn().mockResolvedValue({ id: 'created-id' })  
        const mockUpdate = vi.fn().mockResolvedValue({id: 'updated-id'})
    ...
    test('Should Update a existing published build status if exists and non required parameters are null', async () => {
        const mockCreate = vi.fn().mockResolvedValue({ id: 'created-id' })  
        const mockUpdate = vi.fn().mockResolvedValue({id: 'updated-id'})
    ...
    test('Should fail if no pages Site id is provided', async () => {
        const mockCreate = vi.fn().mockResolvedValue({ id: 'created-id' })  
        const mockUpdate = vi.fn().mockResolvedValue({id: 'updated-id'})
        ...
        expect(result).toMatchObject({ message: 'Pages site id not provided' })
        expect(mockCreate).not.toHaveBeenCalled()
        expect(mockUpdate).not.toHaveBeenCalled()
    })

    test('Should fail if no Build id is provided', async () => {
        const mockCreate = vi.fn().mockResolvedValue({ id: 'created-id' })  
        const mockUpdate = vi.fn().mockResolvedValue({id: 'updated-id'})
        ...
        const result = await response.json()
        expect(response.status).to.equals(400);
        expect(result).toMatchObject({ message: 'Build id not provided' })
        expect(mockCreate).not.toHaveBeenCalled()
        expect(mockUpdate).not.toHaveBeenCalled()
    })

    test('Should fail if no Site id found', async () => {
        const mockCreate = vi.fn().mockResolvedValue({ id: 'created-id' })  
        const mockUpdate = vi.fn().mockResolvedValue({id: 'updated-id'})

...
async function exportHomePage() {
  try {
    ...
    if (homePageDocs.docs.length === 0) {
      console.log('⚠️  No home page found for this site.')
      return
    }
    ...
    console.log('✅ Home page data exported successfully!')
  } catch (error) {
    console.error('❌ Error exporting home page:', error)
  }
}
...
async function exportMenu() {
  try {
    ...
    if (menuDocs.docs.length === 0) {
      console.log('⚠️  No menu found for this site.')
      return
    }
...
async function exportSideNavigation() {
  try {
    ...
    if (sideNavDocs.docs.length === 0) {
      console.log('⚠️  No side navigation found for this site.')
      return
    }
    ...
    fs.writeFileSync(filePath, fileContent, 'utf-8')
    console.log('✅ Side navigation data exported successfully!')
  } catch (error) {
    console.error('❌ Error exporting side navigation:', error)
  }
}

async function exportGlobals() {
  console.log(`📦 Exporting globals for site ID: ${siteId}...\n`)
  await exportHomePage()
  await exportMenu()
  await exportSideNavigation()
  console.log('\n💡 To use this data in seeding, run: npm run dc:seed')
}
